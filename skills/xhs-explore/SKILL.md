...
`文字`、`图文`、`文字+图文`、`text`、`text+image` 都表示非视频笔记。`imageList` 只说明笔记有图片，不等于必须下载或分析图片。

默认不要下载或分析图片。先用搜索结果、笔记正文、互动数据和评论判断价值；`get-feed-detail` 返回的 `note.imageList` 只当作图片存在性信号。只有在用户明确要求包含图片信息，或标题/正文/评论出现强信号时，才进入图片处理。强信号包括：`见图`、`图里`、`截图`、`菜单`、`清单`、`表格`、`地图`、`路线图`、`价格表`、`票价`、`优惠券`、`voucher`、`真题`、`P1/P2`、`第几张图`。
...
当详情中的 `note.imageList` 非空，且用户要求或文本强烈提示图片含关键文字时，才运行：
...
输出包含每张图片的本地路径、OCR 文本和 `combinedText`。OCR 是低成本文字提取，适合截图、菜单、清单、票价、真题、优惠券、表格；不适合判断照片质量、空间布局、地图路线、菜品外观、酒店房型等视觉语义。若视觉语义重要，且当前宿主模型支持原生图像理解，应只挑选少量最相关图片进行视觉分析；否则说明图片需要人工或视觉模型复核。
